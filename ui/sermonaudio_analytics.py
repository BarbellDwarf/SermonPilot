...
class SermonAudioAnalytics:
    ...
    def _parse_sermon_data(self, sermon):
        ...
        try:
            ...
            return {
                ...
                'series': (
                    (
                        sermon.get('series', {}).get('title')
                        if isinstance(sermon.get('series'), dict)
                        else sermon.get('seriesTitle')
                    ) or sermon.get('subtitle', '')
                ),
                ...
                'duration_minutes': (
                    (sermon.get('audioDurationSeconds', 0) // 60)
                    if sermon.get('audioDurationSeconds') else 0
                ),
    ...
    def get_speaker_stats(self) -> list[dict[str, Any]]:
        ...
        # Calculate averages
        for _speaker, stats in speaker_stats.items():
    ...
    def get_series_stats(self) -> list[dict[str, Any]]:
        ...
        # Calculate averages
        for _series, stats in series_stats.items():
            count = stats["sermon_count"]
            stats["avg_views"] = stats["total_views"] / count
            stats["avg_listens"] = stats["total_listens"] / count
