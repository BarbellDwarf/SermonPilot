...
if __name__ == "__main__":
    # Hard skip during automated test runs (requires external Audacity instance)
    if 'PYTEST_CURRENT_TEST' in os.environ:  # pragma: no cover
        raise SystemExit(0)

    print("pipe-test.py (skipped under pytest) detecting platform")
    TONAME = '\\.?\\pipe\\ToSrvPipe' if sys.platform == 'win32' else '/tmp/unused'
    FROMNAME = '\\.?\\pipe\\FromSrvPipe' if sys.platform == 'win32' else '/tmp/unused'
    EOL = '\r\n\0'
    ...
    FROMFILE = open(FROMNAME, encoding='utf-8')
