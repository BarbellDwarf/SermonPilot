...
def test_get_broadcaster_series_reads_title_field(monkeypatch) -> None:
    ...
    assert series == [{"name": "Sample Series", "seriesID": 100001}]
    assert sermon_updater._SERIES_BY_NAME["Sample Series"] == 100001


def test_resolve_series_id_finds_cached_series(monkeypatch) -> None:
    monkeypatch.setitem(sermon_updater._SERIES_BY_NAME, "Sample Series A", 100002)

    assert sermon_updater.resolve_series_id("sample series a") == 100002
...
def test_resolve_series_id_creates_missing_when_allowed(monkeypatch) -> None:
    ...
    created = Mock(return_value=100003)
    ...
    assert sermon_updater._SERIES_BY_NAME["New Series"] == 100003
...
def test_create_series_on_api_parses_created_id(monkeypatch) -> None:
    ...
    response.json.return_value = {
        "seriesID": 100003, "title": "Fresh Series", "broadcasterID": "test"
    }
    monkeypatch.setattr(
        sermon_updater.requests, "post", lambda *a, **k: response
    )

    assert sermon_updater.create_series_on_api("Fresh Series") == 100003
