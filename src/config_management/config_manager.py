...
class SQLConfigManager:
    ...
    def set_config(self, key_path: str, value: Any, changed_by: str, reason: str = None):
        ...
        key_info = cursor.execute("""
            ...
            LEFT JOIN configuration_values v ON k.id = v.key_id AND v.environment = ?
            AND v.is_active = 1
            WHERE k.key_path = ?
    ...
    def import_config(
        self, config_data: str, format: str, imported_by: str, overwrite: bool = False
    ):
    ...
    def get_configuration_history(
        self, key_path: str = None, limit: int = 100
    ) -> list[dict[str, Any]]:
        ...
        for row in rows:
            if row['value'] is not None:
                self._set_nested_value(
                    config, row['key_path'],
                    self._parse_value(row['value'], row['data_type']),
                )
    ...
    def _import_config_recursive(
        self, data: dict[str, Any], parent_path: str, imported_by: str, overwrite: bool
    ):
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                ...
                if key_exists:
                    ...
                    logger.warning(
                        f"Configuration key '{current_path}' not found in schema, skipping"
                    )
