...
class QuestionProcessor:
    ...
    def _detect_with_librosa(self, audio_data: np.ndarray) -> list[tuple[float, float]]:
        ...
        for peak in peaks:
            ...
            segment_end = min(
                len(audio_data) / self.sample_rate, peak_time + 5.0
            )  # 5 seconds after
    ...
    def _calculate_rms_manual(
        self, audio_data: np.ndarray, frame_length: int, hop_length: int
    ) -> np.ndarray:
    ...
    def _analyze_energy_pattern(self, segment: np.ndarray) -> float:
        ...
        rms = self._calculate_rms_manual(
            segment, int(0.05 * self.sample_rate), int(0.025 * self.sample_rate)
        )
    ...
    def _extract_segment(
        self, audio_data: np.ndarray, start_time: float, duration: float
    ) -> np.ndarray:
    ...
    def _extend_segment(
        self, audio_data: np.ndarray, center_time: float, segment: np.ndarray
    ) -> tuple[float, float]:
    ...
    def _find_segment_end(self, audio_data: np.ndarray, current_pos: int) -> int:
        ...
        for i in range(
            current_pos, min(len(audio_data), current_pos + int(5 * self.sample_rate)),
            window_size,
        ):
